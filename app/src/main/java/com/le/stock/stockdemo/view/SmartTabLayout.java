 ...
 */
package com.le.stock.stockdemo.view;
...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    private static final int TAB_VIEW_PADDING_LEFT_DIPS = 16;
    private static final int TAB_VIEW_PADDING_RIGTH_DIPS = 16;
    ...
    private int tabViewTextHorizontalPaddingLeft;
    private int tabViewTextHorizontalPaddingRigth;
    ...
    public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        int textHorizontalPadding = (int) (TAB_VIEW_PADDING_LEFT_DIPS * density);
        int textHorizontalPaddingRigth = (int) (TAB_VIEW_PADDING_RIGTH_DIPS * density);
        ...
        textHorizontalPadding = a.getDimensionPixelSize(
                R.styleable.stl_SmartTabLayout_stl_defaultTabTextHorizontalPaddingLeft, textHorizontalPadding);
        textHorizontalPaddingRigth = a.getDimensionPixelSize(
                R.styleable.stl_SmartTabLayout_stl_defaultTabTextHorizontalPaddingRigth, textHorizontalPaddingRigth);
        ...
        this.tabViewTextHorizontalPaddingLeft = textHorizontalPadding;
        this.tabViewTextHorizontalPaddingRigth = textHorizontalPaddingRigth;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter() && tabStrip.getChildCount() > 0) {
            ...
            int start = (w - SmartTabUtils.getMeasuredWidth(firstTab)) / 2 - SmartTabUtils.getMarginStart(firstTab);
            int end = (w - SmartTabUtils.getMeasuredWidth(lastTab)) / 2 - SmartTabUtils.getMarginEnd(lastTab);
...
//        textView.setTypeface(Typeface.DEFAULT_BOLD);
        ...
        textView.setPadding(
                tabViewTextHorizontalPaddingLeft, 0,
                tabViewTextHorizontalPaddingRigth, 0);
     ...
     * @param tabTexts
     */
    public void setTabText(List<String> tabTexts,int freshTabSelectPostion) {
        tabStrip.removeAllViews();
        tabTextViews=new ArrayList<TextView>();
        for (int i = 0; i < tabTexts.size(); i++) {
            TextView textView=createDefaultTabView(tabTexts.get(i));
            final View tabView;
            if (tabProvider == null){
                tabView=textView;
                tabTextViews.add(textView);
            }else{
                tabView=tabProvider.createTabView(tabStrip, i, tabTexts);
            }
//            final View tabView = (tabProvider == null)
//                    ? textView: tabProvider.createTabView(tabStrip, i, tabTexts);
            if (tabView == null) {
...
//            }
        ...
        if(tabTextViews!=null&&tabTextViews.size()>=1){
            tabTextViews.get(freshTabSelectPostion).setTextColor(Color.parseColor("#2591FF"));
        }
    }
    ...
    private void scrollToTab(int tabIndex, float positionOffset) {
        ...
        final boolean isLayoutRtl = SmartTabUtils.isLayoutRtl(this);
        View selectedTab = tabStrip.getChildAt(tabIndex);
        int widthPlusMargin = SmartTabUtils.getWidth(selectedTab) + SmartTabUtils.getMarginHorizontally(selectedTab);
        ...
        if (tabStrip.isIndicatorAlwaysInCenter()) {
            ...
            if (0f < positionOffset && positionOffset < 1f) {
                ...
                int selectHalfWidth = SmartTabUtils.getWidth(selectedTab) / 2 + SmartTabUtils.getMarginEnd(selectedTab);
                int nextHalfWidth = SmartTabUtils.getWidth(nextTab) / 2 + SmartTabUtils.getMarginStart(nextTab);
            ...
            int x;
            if (isLayoutRtl) {
                int first = SmartTabUtils.getWidth(firstTab) + SmartTabUtils.getMarginEnd(firstTab);
                int selected = SmartTabUtils.getWidth(selectedTab) + SmartTabUtils.getMarginEnd(selectedTab);
                x = SmartTabUtils.getEnd(selectedTab) - SmartTabUtils.getMarginEnd(selectedTab) - extraOffset;
                x -= (first - selected) / 2;
            } else {
                int first = SmartTabUtils.getWidth(firstTab) + SmartTabUtils.getMarginStart(firstTab);
                int selected = SmartTabUtils.getWidth(selectedTab) + SmartTabUtils.getMarginStart(selectedTab);
                x = SmartTabUtils.getStart(selectedTab) - SmartTabUtils.getMarginStart(selectedTab) + extraOffset;
                x -= (first - selected) / 2;
            }
        ...
        if (titleOffset == TITLE_OFFSET_AUTO_CENTER) {
            ...
            if (isLayoutRtl) {
                x = -SmartTabUtils.getWidthWithMargin(selectedTab) / 2 + getWidth() / 2;
                x -= SmartTabUtils.getPaddingStart(this);
            } else {
                x = SmartTabUtils.getWidthWithMargin(selectedTab) / 2 - getWidth() / 2;
                x += SmartTabUtils.getPaddingStart(this);
            }
        ...
        int start = SmartTabUtils.getStart(selectedTab);
        int startMargin = SmartTabUtils.getMarginStart(selectedTab);
        if (isLayoutRtl) {
            x += start + startMargin - extraOffset - getWidth() + SmartTabUtils.getPaddingHorizontally(this);
        } else {
    ...
    public interface TabProvider {
        ...
        View createTabView(ViewGroup container, int position, List<String> tabTexts);
    ...
    private static class SimpleTabProvider implements TabProvider {
        ...
        @Override
        public View createTabView(ViewGroup container, int position, List<String> tabTexts) {
            View tabView = null;
            ...
            if (tabTitleView != null) {
                tabTitleView.setText(tabTexts.get(position));
            }
...
//        viewPagerPageChangeListener.onPageScrolled(position, positionOffset, positionOffsetPixels);
    ...
    public void setTabSelected(int postion){
        tabStrip.getChildAt(postion).setSelected(true);
        tabStrip.onViewPagerPageChanged(postion, 0f);
        for (int i=0;i<tabTextViews.size();i++){
            if (postion==i){
                tabTextViews.get(i).setTextColor(Color.parseColor("#2591FF"));
            }else{
    ...
    private class InternalTabClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            for (int i=0;i<tabTextViews.size();i++){
                if (textView.getText().equals(tabTextViews.get(i).getText())){
                    tabTextViews.get(i).setTextColor(Color.parseColor("#2591FF"));
                }else{
                    tabTextViews.get(i).setTextColor(Color.parseColor("#666666"));
                }
            ...
            for (int i = 0; i < tabStrip.getChildCount(); i++) {
                if (v == tabStrip.getChildAt(i)) {
                    ...
                    if (viewPager != null) {
                        ...
                        scrollToTab(i, 0);
                        currentPostion=i;
//                        for (int j = 0, size = tabStrip.getChildCount(); j < size; i++) {
//                            tabStrip.getChildAt(i).setSelected(j == i);
//                            currentPostion=i;
//                        }
                    }
                    return;
