...
public class MyYAxisRenderer extends YAxisRenderer {
    ...
    /**
     * 图标Y轴Lable从中间开始划分为上红下绿
     */
    private boolean differenColorFromMiddle=false;
     ...
     * @return
     */
    public boolean isDifferenColorFromMiddle() {
        return differenColorFromMiddle;
    }
    /**
     * 设置  图标Y轴Lable从中间开始划分为上红下绿
     * @return
     */
    public void setDifferenColorFromMiddle(boolean differenColorFromMiddle) {
        this.differenColorFromMiddle = differenColorFromMiddle;
    }
    @Override
       ...
       /*当有最小text的时候*/
        if (!TextUtils.isEmpty(mYAxis.getMinValue()) && mYAxis.isShowOnlyMinMaxEnabled()) {
            for (int i = 0; i < mYAxis.mEntryCount; i++) {
                ...
                if (differenColorFromMiddle){
                    if (i<mYAxis.mEntryCount/2){
                        mAxisLabelPaint.setColor(Color.parseColor(green));
                    }else if(i==mYAxis.mEntryCount/2){
                        mAxisLabelPaint.setColor(Color.parseColor(black));
                    }else{
                        mAxisLabelPaint.setColor(Color.parseColor(red));
                    }
                }
