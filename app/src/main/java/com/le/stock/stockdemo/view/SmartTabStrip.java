...
class SmartTabStrip extends LinearLayout {
  ...
  private static final int SELECTED_INDICATOR_THICKNESS_DIPS = 58;
  ...
  private void drawDecoration(Canvas canvas) {
    ...
    if (tabCount > 0) {
      ...
      int selectedStart = SmartTabUtils.getStart(selectedTab, indicatorWithoutPadding);
      int selectedEnd = SmartTabUtils.getEnd(selectedTab, indicatorWithoutPadding);
      int left;
      int right;
      if (isLayoutRtl) {
        left = selectedEnd;
      ...
      if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
        ...
        int nextStart = SmartTabUtils.getStart(nextTab, indicatorWithoutPadding);
        int nextEnd = SmartTabUtils.getEnd(nextTab, indicatorWithoutPadding);
        if (isLayoutRtl) {
...
//    drawSeparator(canvas, height, tabCount);
  ...
  private void drawSeparator(Canvas canvas, int height, int tabCount) {
    ...
    for (int i = 0; i < tabCount - 1; i++) {
      ...
      int end = SmartTabUtils.getEnd(child);
      int endMargin = SmartTabUtils.getMarginEnd(child);
  ...
  private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
    ...
    if (indicatorWidth == AUTO_WIDTH) {
      ...
      indicatorRectF.set(left + padding*3, top, right - padding*3, bottom);
    }
