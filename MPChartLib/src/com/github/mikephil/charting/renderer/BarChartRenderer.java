    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                int i = j / 4;
                if (i > 0) {
                    if (dataSet.getEntryForIndex(i).getData()!=null&&(int)dataSet.getEntryForIndex(i).getData()==0) {//0代表红色
                        mRenderPaint.setColor(Color.RED);
                    } else if (dataSet.getEntryForIndex(i).getData()!=null&&(int)dataSet.getEntryForIndex(i).getData()==1){//1代表绿色
