...
public abstract class DataRenderer extends Renderer {
     ...
     * @param valuePaint
     */
    public void drawMaxMinValue(Canvas c, ValueFormatter formatter, float value, Entry entry, int dataSetIndex, float x, float y,int color,Paint valuePaint){
        mValuePaint.setColor(Color.WHITE);


        //文字背景方块
        Paint textBackPaint=new Paint();
        textBackPaint.setColor(Color.BLACK);


        String str=formatter.getFormattedValue(value, entry, dataSetIndex, mViewPortHandler);
//        float textWidth=mValuePaint.measureText(str);

        Rect rect = new Rect();
        mValuePaint.getTextBounds(str, 0, str.length(), rect);
        int width = rect.width();//文本的宽度
        int height = rect.height();//文本的高度

        float xOffset = Utils.convertDpToPixel(15f);//设置在X轴方向上的额外偏移
        float m=0;//text显示的起始位置X轴方向
        float middlePort=mViewPortHandler.contentLeft()+(mViewPortHandler.contentRight()-mViewPortHandler.contentLeft())/2;
        if (x<=middlePort){//在图表中线线的左边
            m=x-width-xOffset;
            if (m<mViewPortHandler.contentLeft()){
                m=x+xOffset;
            }

        }else{//在图表中线线的右边
            m=x+xOffset;
            if(m+width>mViewPortHandler.contentRight()){
                m=x-width-xOffset;
            }
        }
        c.drawLine(x,y,m,y,textBackPaint);

//        if (x!=0){
//        }
//        String text = "Android";

        c.drawRect(m,y-height,m+width,y+height,textBackPaint);

        c.drawText(str, m+width/2, y+height/2, mValuePaint);
    }
    /**
