...
public class CandleStickChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    private final boolean isDrawMaxMinValue=true;
    @Override
...
//                * mViewPortHandler.getScaleX()) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                int  minx= Math.max(mMinX, 0);//from
                int maxx = Math.min(mMaxX + 2, dataSet.getEntryCount());//to

               float[] positions = trans.getMaxValues(
                        dataSet, mAnimator.getPhaseX(), mAnimator.getPhaseY(), minx, maxx);
                float[] positionsMin = trans.getMinValues(
                        dataSet, mAnimator.getPhaseX(), mAnimator.getPhaseY(), minx, maxx);

                float[] gpositions = trans.generateTransformedValuesCandle(
                        dataSet, mAnimator.getPhaseX(), mAnimator.getPhaseY(), minx, maxx);
                float[] gpositionsMin = trans.generateTransformedValuesCandleMin(
                        dataSet, mAnimator.getPhaseX(), mAnimator.getPhaseY(), minx, maxx);


                if (isDrawMaxMinValue){
                    int start=0;//屏幕中当前显示的view中---左边开始位置
                    int end=0;//屏幕中右边的结束位置
                    boolean xStart=false,xEnd=false;
                    //计算出当前屏幕显示的开始点和结束点的数组下标
                    int maxPostion=0;
                    int minPostion=0;
                    float max=Float.MIN_VALUE;
                    float min=Float.MAX_VALUE;
                    float xOffset = Utils.convertDpToPixel(0f);//设置在x轴方向上的额外偏移
                    for (int m = 0; m< positions.length; m+=2) {

                        float x = gpositions[m];//改点x方向未经放大缩小位移时的值
                        float y=positions[m+1];
                        if (x*mAnimator.getPhaseX()<=mViewPortHandler.contentLeft()+xOffset||x*mAnimator.getPhaseX()>=mViewPortHandler.contentRight()-xOffset){
                            continue;
                        }
                        if (max<=y){
                            max=y;
                    ...
                    for (int m = 0; m< positionsMin.length; m+=2) {
                        float x = gpositions[m];//改点x方向未经放大缩小位移时的值
                        float y=positionsMin[m+1];
                        if (x*mAnimator.getPhaseX()<mViewPortHandler.contentLeft()+xOffset||x*mAnimator.getPhaseX()>mViewPortHandler.contentRight()-xOffset){
                            continue;
                        }
                        if (min>=y){
                    ...
                    //根据最大最小值的下标，绘制数据值
//                    for (int j = start; j < (end-1)*2; j += 2) {
//                        float x = positions[j];//改点x方向未经放大缩小位移时的值
//                        float yHighMax = positions[j + 1];
//                        float yHighMin = positionsMin[j + 1];
////                        float yLow=positions[j+2];
//
//                        if (!mViewPortHandler.isInBoundsRight(x))
//                            break;
//
//                        if (!mViewPortHandler.isInBoundsLeft(x) || !mViewPortHandler.isInBoundsY(yHighMax)||!mViewPortHandler.isInBoundsY(yHighMin))
//                            continue;
//
//                        CandleEntry entry = dataSet.getEntryForIndex(j / 2 + minx);
//                        if (j==maxPostion){
//                            drawMaxMinValue(c, dataSet.getValueFormatter(), entry.getHigh(), entry, i, x, yHighMax, dataSet.getValueTextColor(j / 2),null);
//                        }
//                        if (j==minPostion){
//                            drawMaxMinValue(c, dataSet.getValueFormatter(), entry.getHigh(), entry, i, x, yHighMin, dataSet.getValueTextColor(j / 2),null);
//                        }
//                    }
                    //画最大值
                    CandleEntry entryMax = dataSet.getEntryForIndex(maxPostion/2 + minx);
                    float xMax = gpositions[maxPostion];//改点x方向未经放大缩小位移时的值
                    float yHighMax = gpositions[maxPostion+1];
                    drawMaxMinValue(c, dataSet.getValueFormatter(), entryMax.getHigh(), entryMax, maxPostion, xMax, yHighMax, dataSet.getValueTextColor(maxPostion),null);
                    //画最小值
                    CandleEntry entryMin = dataSet.getEntryForIndex(minPostion/2 + minx);
                    float xMin = gpositionsMin[minPostion];//改点x方向未经放大缩小位移时的值
                    float yHighMin = gpositionsMin[minPostion+1];
                    drawMaxMinValue(c, dataSet.getValueFormatter(), entryMin.getLow(), entryMin, minPostion, xMin, yHighMin, dataSet.getValueTextColor(minPostion),null);

                }else{
                    float yOffset = Utils.convertDpToPixel(5f);//设置在Y轴方向上的额外偏移
                    for (int j = 0; j < positions.length; j += 3) {
                        ...
                        CandleEntry entry = dataSet.getEntryForIndex(j / 3 + minx);

                        drawValue(c, dataSet.getValueFormatter(), entry.getHigh(), entry, i, x, y - yOffset, dataSet.getValueTextColor(j / 3));
                    }
                }
            }
        }
